...
for file in os.listdir(directory):
    ...
    mace_calc = mace_mp(device='cpu', default_dtype="float32")
    ...
    try:
        db.get(a=a,b=b,c=c, opt_converged=True, model='mace-mpcalculator')
    except KeyError:
        relax(atoms, run_name)
        print("***Total Energy from MACE:", a, b, c, atoms.get_potential_energy())
        db.write(atoms,a=a, b=b, c=c, model='mace-mpcalculator', opt_converged=atoms.info['opt_converged'])       
